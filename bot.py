...
TOKEN = os.environ.get("8627933053:AAGhQyyblUP237We1QRMMwTBY7IW45SK79Y")
...
# مراحل ثبت سفارش مکالمه‌ای و ارسال پیام
PROJECT_TYPE, USER_NAME, USER_PHONE = range(3)
ADMIN_MESSAGE = range(1)

# راه‌اندازی سرور فلاسک برای پایداری روی رندر و وب‌هوک
app = Flask(__name__)
...
# مدیریت کلیک دکمه‌ها و نمایش آثار با شناسه‌های عکس
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # دسته‌بندی ۱: سریال‌ها و فیلم‌های داستانی
    ...
    # دسته‌بندی ۳: پروژه ملی و کتاب گاز
    elif data == "port_gas":
        keyboard = [
    ...
    # نمایش آثار با شناسه‌های عکس (File IDs)
...
# ابزار کمکی برای دریافت خودکار شناسه عکس‌ها در صورت نیاز به تست‌های آتی
async def get_file_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        photo_file_id = update.message.photo[-1].file_id
        await update.message.reply_text(
            f"📸 **شناسه این عکس دریافت شد:**\n\n`{photo_file_id}`\n\nاین مقدار را می‌توانید کپی کنید.",
            parse_mode="Markdown"
        )

def main():
    # راه‌اندازی سرور وب در یک ترد جداگانه برای رندر
    ...
    logger.info("Bot is running successfully with all complete portfolio photo IDs!")
    
    # استفاده از Webhook یا Polling بر اساس نیاز استقرار روی Render
